  ...
  for (const type of Object.keys(byType)) {
    ...
    try {
      ...
      for (const row of data.results) {
        ...
        // Période : même logique de repli de colonnes que côté page (voir
        // pickKeyPriority dans index.html), pour les mêmes variantes selon
        // le type de bouée.
        const period = pickField(entete, row, /^(TH1\/3|T02|TH13D|TP)/i);
        const threshold = spot.threshold || 1.5;
        const thresholdPeriod = spot.thresholdPeriod || 8;
        // Alerte double : hauteur ET période doivent CHACUNE dépasser leur
        // seuil (voir statusFor() dans index.html — même règle des deux côtés).
        const heightMet = height >= threshold;
        const periodMet = period !== null && period >= thresholdPeriod;
        if (heightMet && periodMet) {
          await self.registration.showNotification("Alerte houle 🌊", {
            body: `${spot.nom} : ${height.toFixed(2)} m / ${period.toFixed(1)} s (seuils ${threshold} m, ${thresholdPeriod} s)`,
